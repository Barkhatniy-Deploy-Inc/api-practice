...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS sync_history (
            region_code TEXT, year INTEGER, month INTEGER,
            records_downloaded INTEGER, sync_date DATETIME DEFAULT CURRENT_TIMESTAMP,
            PRIMARY KEY (region_code, year, month)
        )
    ''')

    # Обновленные округа с кодами для бэкенда
    districts = [
            ...
            for card in dtp_cards:
                ...
                for ts in ts_info:
                    ...
                    for uch in ts_uch:
                        ...
                        # --- АГРЕССИВНОЕ ИЗВЛЕЧЕНИЕ ВОЗРАСТА И СТАЖА ---
                        raw_age = str(uch.get('v_r') or uch.get('V_R') or '0')
                        age_digits = ''.join(filter(str.isdigit, raw_age))
                        age = int(age_digits) if age_digits else 0
                        
                        raw_exp = str(uch.get('v_st') or uch.get('V_ST') or '0')
                        exp_digits = ''.join(filter(str.isdigit, raw_exp))
                        experience = int(exp_digits) if exp_digits else 0
                        # -----------------------------------------------
...
def main():
    ...
    logging.info("🚀 Синхронизация запущена (Финальная версия с агрессивным парсингом)...")
